...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Imaar.Blazor` and `Imaar.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p cfce65e6-8592-4687-ba4f-38234ef51d6a
```

> `cfce65e6-8592-4687-ba4f-38234ef51d6a` is the password of the certificate, you can change it to any password you want.
...
Run `Imaar.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Imaar.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Imaar.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
